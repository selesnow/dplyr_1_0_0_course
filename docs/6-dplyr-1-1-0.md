...
1. **Аргумент `.by`:** В чем заключается главное отличие использования временной группировки через аргумент `.by` от классического метода `group_by()`?
2. **Сложные джойны:** Какую новую вспомогательную функцию необходимо использовать внутри `inner_join()` или `left_join()`, чтобы выполнить соединение таблиц по неравенству (например, когда дата в одной таблице должна быть больше даты в другой)?
3. **Функция `reframe()`:** Для какой специфической задачи была введена функция `reframe()`, и почему для этого теперь не рекомендуется использовать `summarise()`?
4. **Производительность `arrange()`:** Какое техническое изменение произошло в работе функции `arrange()` в версии 1.1.0 и как это отразилось на скорости обработки данных?
